...
@keyframes countdownText{
	...
	40% {transform: scale3d(.9,.9,1)}
	60% {
		opacity: 1;
		transform: scale3d(1.03,1.03,1)
	}
	80% {transform: scale3d(.97,.97,1)}
...
.lottoMachine__yeekee{
  ...
  &__frame{
    ...
    &__num{
      ...
      &:nth-of-type(1), &:nth-of-type(4){box-sizing: content-box;}
			&:nth-of-type(1){ // left top/寬度 高度
				background: url("@/assets/img/lottoMachine/screenLotteryNumLbg.png") no-repeat 0 0/23% 100%;
				padding-left: size_pxToVw(5,$screenWidth);
			}
			&:nth-of-type(4){
				background: url("@/assets/img/lottoMachine/screenLotteryNumRbg.png") no-repeat 100% 0/23.3766% 100%;
				padding-right: size_pxToVw(4.5,$screenWidth);
			}
      &-small{
        display:flex;
        flex: 0 1 size_pxToVw(34,$screenWidth);
        perspective:size_pxToVw(50*1.5,$screenWidth);
        &:after{
          height: size_pxToVw(49,$screenWidth);
          background-image:url("@/assets/img/lottoMachine/screenRightSideNumS.png");
          margin: auto 0;
        }
      }
      &-pig{
  ...
  &Ready{
    ...
    .lottoMachine__yeekee__frame__num{
      &:after{
        transform:rotate3d(0,0,0,0deg);
        animation: yeekeeNumGo 1.5s linear infinite both;
      }
      @for $i from 1 through 9{
        &:nth-of-type(#{$i}):after{animation-delay: $i * .3 + .3s;}
      }
      @for $i from 2 through 9{
        &:nth-of-type(#{$i}):after{transition-delay:($i - 1) * .3s;}
      }
